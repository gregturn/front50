...
//    "someMap"                        | [ key1: "a", key2: 2, nested: [ subkey: "33", something: true]] // TODO Handle maps within maps -> https://jira.spring.io/browse/DATAREDIS-489?focusedCommentId=128546&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel#comment-128546
  ...
  def "should report failing redis connection as not healthy"() {
    given:
    redisApplicationDAO.factory = Mock(RedisConnectionFactory)
    ...
    1 * redisApplicationDAO.factory.getConnection() >> { throw new RuntimeException('Failed') }
    0 * redisApplicationDAO.factory._
  }
  ...
  void deleteAll() {
    redisApplicationDAO.redisTemplate.delete(redisApplicationDAO.redisTemplate.keys(RedisApplicationDAO.key('*')))
    redisApplicationDAO.stringRedisTemplate.delete(RedisApplicationDAO.bookkeepingKey())
  }
