...
class RedisProjectDAOSpec extends Specification {
  ...
  def "should report failing redis connection as not healthy"() {
    given:
    redisProjectDAO.factory = Mock(RedisConnectionFactory)
    ...
    1 * redisProjectDAO.factory.getConnection() >> { throw new RuntimeException('Failed') }
    0 * redisProjectDAO.factory._
  }
