...
class RedisPipelineStrategyDAOSpec extends Specification {
  ...
  def "should report failing redis connection as not healthy"() {
    given:
    redisPipelineStrategyDAO.factory = Mock(RedisConnectionFactory)
    ...
    1 * redisPipelineStrategyDAO.factory.getConnection() >> { throw new RuntimeException('Failed') }
    0 * redisPipelineStrategyDAO.factory._
  }

  void deleteAll() {
    redisPipelineStrategyDAO.redisTemplate.delete(redisPipelineStrategyDAO.redisTemplate.keys(RedisPipelineStrategyDAO.key('*')))
    redisPipelineStrategyDAO.stringRedisTemplate.delete(RedisPipelineStrategyDAO.bookkeepingKey())
  }
