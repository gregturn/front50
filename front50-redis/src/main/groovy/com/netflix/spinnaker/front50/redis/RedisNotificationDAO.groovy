...
class RedisNotificationDAO implements NotificationDAO {
  ...
  Notification get(HierarchicalLevel level, String name) {
    redisTemplate.opsForHash().get(bookkeepingKey(), name) ?: [email: []] as Notification // an empty Notification is expected for applications that do not exist
  }
  ...
  void save(HierarchicalLevel level, String name, Notification notification) {
    ...
    redisTemplate.opsForHash().put(bookkeepingKey(), name, notification)
  }

  @Override
  void delete(HierarchicalLevel level, String name) {
    redisTemplate.opsForHash().delete(bookkeepingKey(), name)
  }

  static String bookkeepingKey() {
    'com.netflix.spinnaker:front50:notifications'
  }
